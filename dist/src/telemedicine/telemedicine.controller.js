"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TelemedicineController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const telemedicine_service_1 = require("./telemedicine.service");
const jwt_auth_guard_1 = require("../common/guards/jwt-auth.guard");
const current_user_decorator_1 = require("../common/decorators/current-user.decorator");
let TelemedicineController = class TelemedicineController {
    constructor(telemedService) {
        this.telemedService = telemedService;
    }
    startSession(body, doctorId) {
        return this.telemedService.createSession({
            examinationId: body.examinationId,
            doctorId,
            nurseId: body.nurseId,
        });
    }
    endSession(sessionId, body) {
        return this.telemedService.endSession(sessionId, body.notes, body.recordingKey);
    }
    getSessionHistory(examinationId) {
        return this.telemedService.getSessionHistory(examinationId);
    }
};
exports.TelemedicineController = TelemedicineController;
__decorate([
    (0, common_1.Post)('session/start'),
    (0, swagger_1.ApiOperation)({ summary: 'Shifokor va Qishloq Hamshirasi o‘rtasida jonli video/audio konsultatsiya xonasini ochish' }),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, current_user_decorator_1.CurrentUser)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], TelemedicineController.prototype, "startSession", null);
__decorate([
    (0, common_1.Post)('session/:id/end'),
    (0, swagger_1.ApiOperation)({ summary: 'Konsultatsiyani yakunlash va xulosalarni saqlash' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object]),
    __metadata("design:returntype", void 0)
], TelemedicineController.prototype, "endSession", null);
__decorate([
    (0, common_1.Get)('sessions/:examinationId'),
    (0, swagger_1.ApiOperation)({ summary: 'Tekshiruv bo‘yicha o‘tkazilgan telemeditsina konsultatsiyalari tarixi' }),
    __param(0, (0, common_1.Param)('examinationId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], TelemedicineController.prototype, "getSessionHistory", null);
exports.TelemedicineController = TelemedicineController = __decorate([
    (0, swagger_1.ApiTags)('Telemedicine & Live Video Consultations'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    (0, common_1.Controller)('telemedicine'),
    __metadata("design:paramtypes", [telemedicine_service_1.TelemedicineService])
], TelemedicineController);
//# sourceMappingURL=telemedicine.controller.js.map